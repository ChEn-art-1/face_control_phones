...
class FaceAnalyzer(
    ...
    companion object {
        ...
        private const val LONG_BLINK_MIN_MS = 1200L       // 1200~2000ms 主动长闭眼（约1.5秒）
        private const val LONG_BLINK_MAX_MS = 2000L       // > 2000ms 视为闭眼休息，忽略
    ...
    data class Thresholds(
        ...
        var nodRatio: Float = 0.6f,          // 低头阈值
        var lookUpRatio: Float = 0.35f,      // 抬头阈值
    ...
    private fun handleBlink(eyesClosedNow: Boolean) {
        ...
        if (isEyesClosed) {
            ...
            when {
                ...
                duration in LONG_BLINK_MIN_MS..LONG_BLINK_MAX_MS -> {
                    // 1200~2000ms（约1.5秒）主动长闭眼
                    lastPhysioBlinkTimestamp = 0L
                    onActionDetected(FaceAction.LONG_BLINK)
                }
                else -> {
                    // <1200ms 非长闭或 >2000ms 视为休息，忽略
                    lastPhysioBlinkTimestamp = 0L
                }
            }
        }
